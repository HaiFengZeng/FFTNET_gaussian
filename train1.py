...
from fftnet_gaussian import FFTNet
...
def eval_model(epoch, writer, x, condition, model, device):
    ...
    for i in tqdm(range(condition.size(-1) - model.receptive_field)):
        ...
        if hparams.out_type == 'Gaussian':
            ...
            sample = torch.clamp(sample,min=-1,max=1.0)
        else:
            output = output[0, :, -1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='ljspeeh',
                        help='Metadata file which contains the keys of audio and melspec')
    parser.add_argument('--ema_decay', type=float, default=0.9999,
                        help='Moving average decay rate.')
    parser.add_argument('--num_workers', type=int, default=4,
                        help='Number of dataloader workers.')
    parser.add_argument('--resume', type=str, default='./checkpoints_gaussian/model.ckpt-150000.pt',
                        help='Checkpoint path to resume')
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints_gaussian/',
                        help='Directory to save checkpoints.')
    parser.add_argument('--log_dir', type=str, default='log/gaussian',
                        help='Directory to save checkpoints.')
